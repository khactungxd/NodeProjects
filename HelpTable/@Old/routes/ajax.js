	...
	if(req.query["action"]=="editAField"){		
		...
		sequelize.query("ALTER TABLE `"+tableName+"` CHANGE `"+oldFieldName+"` `"+newFieldName+"` "+fieldType+" NOT NULL").success(function(data) {
			...
			sequelize.query("DESCRIBE `"+tableName+"` ;").success(function(arrFields){							
				...
				sequelize.query("SELECT * FROM `tb_flag` WHERE xTableName='"+tableName+"';").success(function(data){
					var flagExist=0;
					for (var fieldName in arrFields){
						if (fieldName==newFieldName){ flagExist=1; break; }						
					}
					if (flagExist){
						sequelize.query("UPDATE `tb_flag` SET xFlag="+flag+" WHERE xTableName='"+tableName+"' AND xFieldName='"+newFieldName+"';").success(function(data){
							_editAField();
						})	
					} else {
						sequelize.query("INSERT INTO `tb_flag` (xTableName, xFieldName, xFlag) VALUES ('"+tableName+"', '"+newFieldName+"', "+flag+");").success(function(data){								
							_editAField();
						})					
					}
					res.send("success");
