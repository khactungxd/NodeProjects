...
function refreshEventHandlers(){
	$('.btUpdate').click(function(){
		...
		var flag=''+fFilter+fVisual+fResult;		
		
		var ajaxQuery="ajax?tableName="+tbl+"&action=editAField&flag="+flag+"&oldFieldName="+oldFieldName+"&fieldName="+fieldName+"&fieldType="+fieldType+"";
	...
	$('#btAdd').click(function(){		
		...
		if (duplicatedName){
			...
			var ajaxQuery="ajax?tableName="+tbl+"&action=addAField&fieldName="+fieldName+"&fieldType="+fieldType;		
			$('#data').html("<tr><td><img src='images/ajax-loader.gif' style='border:0px' /></td></tr>");		
			$.get(ajaxQuery,function(data){			
				reloadPage();
			});
...
function reloadPage(){
	$.get("ajax?tableName="+tbl+"&action=getTableStructure",function(data){			
		...
		for (var attribute in data){
			...
			html+="</select></td>";
			// FILTER ?
			html+="<td><input type='checkbox' class='cbFilter'  index='"+index+"'  /></td>"; 
			// VISUAL ?
			html+="<td><input type='checkbox' class='cbVisual'  index='"+index+"'  /></td>"; 
			// RESULT ?
			html+="<td><input type='radio' name='radioResult'  class='radioResult' index='"+index+"'  /></td>"; 
