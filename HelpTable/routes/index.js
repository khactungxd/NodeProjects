...
exports.showSample = function(req, res){
	var tbl=req.query['tbl'];	
	var arrParams={ tbl: tbl}; 
	
	res.render('sample', { title: 'HelpTable sample', arrParams: arrParams });
};
exports.downloadCSV = function(req, res){
	...
	sequelize.query("DESCRIBE `"+tableName+"`").success(function(arr){
		...
		sequelize.query("SELECT * FROM `"+tableName+"`").success(function(arrRecords) {
			...
			var path = "./public/files/"+tableName+".csv";
			fs.writeFile(path, content, function (err) {
				if(err) { 
					console.log(err);
				} else {
					console.log("The file was saved!");					
					res.redirect(CONFIG.RELATIVE_PATH+'/files/'+tableName+'.csv');
					res.end();
				}				
			});
		});
